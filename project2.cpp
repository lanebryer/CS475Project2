...
int main(int argc, char *argv[])
{
         ...
         double performance = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.);
          #ifdef coarse
            #ifdef staticRun
              printf("Run mode: Coarse static\nNumber of threads: %d\nPerformance: %8.2lf MegaBodies compared per second\n\n", NUMTHREADS, performance);
            #endif
            #ifdef dynamicRun
              printf("Run mode: Coarse dynamic\nNumber of threads: %d\nPerformance: %8.2lf MegaBodies compared per second\n\n", NUMTHREADS, performance);
            #endif
          #endif
          #ifdef fine
              #ifdef staticRun
                printf("Run mode: Fine static\nNumber of threads: %d\nPerformance: %8.2lf MegaBodies compared per second\n\n", NUMTHREADS, performance);
              #endif
              #ifdef dynamicRun
                printf("Run mode: Fine dynamic\nNumber of threads: %d\nPerformance: %8.2lf MegaBodies compared per second\n\n", NUMTHREADS, performance);
              #endif
          #endif
         return 0;
}
